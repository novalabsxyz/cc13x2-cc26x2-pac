...
impl R {
    ...
    #[doc = "Bit 0 - 0:0\\] Debug freeze enable. 0: AUX_TIMER2 does not halt when the system CPU halts in debug mode. 1: Halt AUX_TIMER2 when the system CPU halts in debug mode."]
    #[inline]
    pub fn dbg_freeze_en(&self) -> DBG_FREEZE_ENR {
        let bits = {
            const MASK: bool = true;
            const OFFSET: u8 = 0;
